<?php
	...
	if($query->rowCount() > 0)
	{
		...
		echo "<button type='button' id='delete_btn' style='display:none' onclick=\"multi_del({$page})\" class='waves-effect waves-light btn'>Delete</button>";
		...
		foreach($query as $r)
		{
			            echo "<tr>
                              ...
                              <td><button type='button' onclick=\"delete_state({$r['id']},{$page})\" class='waves-effect waves-light btn'><i class='fa fa-trash'></i></button></td>
                          </tr>";
		}
		echo "</tbody>";
		echo "</table>";
							...
							echo "<div class='col s12'>";
								echo "<div align='center'id='paginate1' class='pagination'>";																
									...
									if($total_num_pages>1)
									{
										...
										if($previous>=1)
										{
											echo "<li class='waves-effect'><a href='#{$previous}' onclick=\"paginate_state({$previous})\"><i class='material-icons'>chevron_left</i></a></li>";
										}
										for($i=1;$i<=$total_num_pages;$i++)
										{
											...
											{
												echo "<li class='waves-effect'><a href='#{$i}' onclick=\"paginate_state({$i})\">{$i}</a></li>";
											}
										}									
										if($next<=$total_num_pages)
										{
											echo "<li class='waves-effect'><a href='#{$next}' onclick=\"paginate_state({$next})\" data-role='button'><i class='material-icons'>chevron_right</i></a></li>";
										}
									}
								echo "</div>";
							echo "</div>";
